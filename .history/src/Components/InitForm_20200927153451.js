...
export default function InitForm({ setLoad, setTournamentName, setNumGioc, setTipo, setInitForm, setInitForm2, setAudio }) {
    ...
    const caricaTorneoDaFile = (e) => {
        if(window.cordova){
            window.requestFileSystem(window.cordova.LocalFileSystem.PERSISTENT, 0, function (fs) {
                ...
                fs.root.getFile("newPersistentFile.txt", { create: true, exclusive: false }, function (fileEntry) {
                    ...
                    window.cordova.writeFile(fileEntry, null);
            
                }, window.cordova.onErrorCreateFile);
            
            }, window.cordova.onErrorLoadFs);
        }else{
