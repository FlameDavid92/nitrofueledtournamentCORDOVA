...
export default function MyTournament({writeFile, goToHome, setAudio, setInitForm, setInitForm2, setTournamentName, setNumGioc, setTipo, setPlayers, tournamentName, load, tipo, numGioc, battles, tracks, players }) {
    ...
    useEffect(() => {
        (pointsChanged !== 0) && saveToLocalStorage();
    },[pointsChanged])
    ...
    return (
        <>
            ...
            <MyModal2 pointsChanged={pointsChanged} setPointsChanged={setPointsChanged} title="Modifica Punti" myTournament={myTournament ? myTournament : []} setMyTournament={setMyTournament} pointsChangeId={pointsChangeId} players={players} setPlayers={setPlayers} clicksModal2={clicksModal2}></MyModal2>
        </>
    )
}